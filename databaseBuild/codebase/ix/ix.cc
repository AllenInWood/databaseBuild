...
RC IndexManager::insertOnIndexPage( IXFileHandle &ixfileHandle, void *memory, const void *key, AttrType type )
{
    short int   flag        = *(short int *) ( (char *) memory + 3);
    short int   numberOfNodes   = *(short int *) ( (char *) memory + 1);
    short int   moveBegin, moveEnd;
    ...
    if ( type == TypeVarChar )
    {
        ...
        /* Move the data */
        int newPageNumber = ixfileHandle.fileHandle.getNumberOfPages() - 1;
        if ( moveBegin != moveEnd )
        {
            void *data = malloc( moveEnd - moveBegin );
            memcpy( data, (char *) memory + moveBegin, moveEnd - moveBegin );
...
RC IndexManager::updateIndexPage( IXFileHandle &ixfileHandle, void *memory, const void *key, AttrType type, void *midKey, bool leftIndicator )
{
    short int   numberOfNodes   = *(short int *) ( (char *) memory + 1);
    short int   flag        = *(short int *) ( (char *) memory + 3);
    numberOfNodes--;
    ...
    memcpy( (char *) memory + 1, &numberOfNodes, sizeof(short int) );
    memcpy( (char *) memory + 3, &flag, sizeof(short int) );
    insertOnIndexPage( ixfileHandle, memory, key, type );
    return(0);
}
            ...
            /* If not, split the leaf */
                ...
                if ( trace.empty() )
                {
                    ...
                    if ( midKeyLength + 6 <= flag )
                    {
                        ...
                        insertOnIndexPage( ixfileHandle, memory, midKey, attribute.type );
                        ixfileHandle.writePage( page, memory );
                    }else  {
                        while ( midKeyLength + 6 > flag )
                        {
                            ...
                            {
                                updateIndexPage( ixfileHandle, memory, lowLayerKey, attribute.type, midKey, true );
                            }
                            ...
                            {
                                updateIndexPage( ixfileHandle, newPage, lowLayerKey, attribute.type, midKey, false );
                            }
                            /* Between the left last key and the right first key, then it is the mid key which will be upgraded to the upper layer */
                                ...
                                int newPointer = ixfileHandle.fileHandle.getNumberOfPages() - 1;
                            ...
                            if ( trace.empty() )
                            {
                                ...
                                ixfileHandle.readPage( (PageNum) trace.back(), memory );
...
/*
 *        if (*(int *)((char *)memory + offset) > 4900) {
 *              cout <<"filename :" << ixfileHandle.indexFileName << endl;
 *              cout <<"int 666 scan:"<<*(int *)((char *)memory + offset) << " " <<*(int *)((char *)highKey) << endl;
 *              int n = *(int *)((char *)memory + offset);
 *              if (n == 5000) {
 *                      cout<< "out";
 *              }
 *        }
        ...
        if ( highKey == NULL )
        {
            ...
            if ( highKeyInclusive )
            {
                ...
                while ( equal == true )
                {
                    if ( index + 1 <= numberOfNodes )
                    {
                        /* cout << "directory offset :" << *(short int *)((char *)memory + PAGE_SIZE - 4 - (index + 1) * 2)  << endl; */
                        rid.pageNum = *(unsigned *) ( (char *) memory + offset + entryVarCharLength);
                        rid.slotNum = *(unsigned *) ( (char *) memory + offset + entryVarCharLength + 4);
                        /* cout << "hpageNum : " << rid.pageNum << " hslotNum : " << rid.slotNum << endl; */
                        ix_ScanIterator.returnedRID.push_back( rid );
                        vector<char> vKey;
                        for ( int i = 0; i < entryVarCharLength; i++ )
                        {
