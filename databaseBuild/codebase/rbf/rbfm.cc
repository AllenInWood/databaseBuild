...
RC RecordBasedFileManager::recordConverter( FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, void *record, short int &recordSize )
{
...
RC RecordBasedFileManager::recordWrapper( FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *record, void *data, const short int recordSize, int &newSize )
{
...
RC RecordBasedFileManager::insertRecord( FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const void *data, RID &rid )
{
	...
	int		i, j;
	recordConverter( fileHandle, recordDescriptor, data, record, recordSize );
...
RC RecordBasedFileManager::readRecord( FileHandle &fileHandle, const vector<Attribute> &recordDescriptor, const RID &rid, void *data )
{
	...
	int newSize = 0;
	recordWrapper( fileHandle, recordDescriptor, pageReader, data, length, newSize );
	free( memory );
	free( pageReader );
	return(0);
}
	...
	}else if ( recordSize < length ) /* smaller */
	{
		...
		if ( flag >= diff )
		{
			...
			if ( firstOffset == maxOffset )
			{
				...
				memcpy( (char *) memory + head, record, recordSize );
				/* memset((char *)memory + head + recordSize + maxOffset - firstOffset, 0, diff); */
		...
		for ( int j = 0; j < numberOfSlots; j++ )
		{
			...
			if ( compOp != NO_OP )
			{
				...
				if ( Comp( pageReader, value, type, compOp ) )
				{
					...
					int		newSize;
					vector<char>	vData;
					recordWrapper( fileHandle, attrs, record, data, recordSize, newSize );
